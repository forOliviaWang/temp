...
class Solver(object):
    def __init__(self, args):
        ...
        # Basic
        self.cuda = (args.cuda and torch.cuda.is_available())  #torch.cuda.is_available()检查pytorch中是否有可用的cuda
        ...
        self.ckpt_dir = Path(args.ckpt_dir).joinpath(args.env_name)  #拼接路径
        if not self.ckpt_dir.exists():
            self.ckpt_dir.mkdir(parents=True, exist_ok=True)   #pathlib的mkdir接收两个参数：parents：如果父目录不存在，是否创建父目录。exist_ok:只有在目录不存在时创建目录，目录已存在时不会抛出异常
    ...
    def visualization_init(self, args):
        # Visdom
        if self.visdom:   #如果在pytorch中使用visdom可视化工具
    ...
    def model_init(self, args):
        # Network  #定义网络时一般是继承torch.nn.Model创建新的子类，此处在ToyNet中定义
        self.net = cuda(ToyNet(y_dim=self.y_dim), self.cuda) #utils.utils中的cuda(tensor,is_cuda),如果is_cuda，则返回tensor.cuda()，否则返回tensor，GPU上的tensor加.cuda转换为GPU上的tensor类型
        self.net.weight_init(_type='kaiming')

        # Optimizers
        self.optim = optim.Adam([{'params':self.net.parameters(), 'lr':self.lr}],
                                betas=(0.5, 0.999))    #betas (Tuple[float, float], optional)：用于计算梯度的平均和平方的系数(默认: (0.9, 0.999))
    ...
    def FGSM(self, x, y_true, y_target=None, eps=0.03, alpha=2/255, iteration=1):
        ...
        x = Variable(cuda(x, self.cuda), requires_grad=True)    #autograd包是pytorch所有神经网络的核心，autograd.Variable是包的中央类，包含一个tensor，创建一个变量，require_grad指定该变量是否计算梯度
        ...
        accuracy = torch.eq(prediction, y_true).float().mean()     #torch.eq(input, other, out=None)比较元素相等性，输出一个Tensor，包含了每个位置的比较结果（相等为1，不等为0）
        ...
        changed = changed.float().view(-1, 1, 1, 1).repeat(1, 3, 28, 28)   #.view()将多维度的tensor展成一维。repeat()沿指定维度重复tensor中的数据

...
class Attack(object):
    ...
    def fgsm(self, x, y, targeted=False, eps=0.03, x_val_min=-1, x_val_max=1):
        ...
        self.net.zero_grad()    #所有参数梯度清零
        if x_adv.grad is not None:   #Variable有三个属性：data：存储了Tensor，是本体的数据；grad：保存了data的梯度，是个Variable而非Tensor，与data形状一致；grad_fn：指向Function对象，用于反向传播的梯度计算之用
            x_adv.grad.data.fill_(0)   
        cost.backward()
        #x_adv:对抗样本
        x_adv.grad.sign_()
        x_adv = x_adv - eps*x_adv.grad
        x_adv = torch.clamp(x_adv, x_val_min, x_val_max)  #torch.clamp(input,min,max,out=None)将input中的元素限制在[min,max]范围，返回一个Tensor
